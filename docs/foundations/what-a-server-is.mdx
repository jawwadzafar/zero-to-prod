...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        'The backend is faster',
        'Code running on the user’s device can be changed by the user, so it cannot be trusted to enforce rules',
        'Frontends cannot display errors',
        'Rules are only allowed in databases',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Redis holds data Postgres does not have',
        'Redis keeps hot data in memory, so most lookups avoid the slower database',
        'Postgres is only for backups',
        'Redis is required by HTTP',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        ...
        'DNS failed to find localhost',
        'Nothing is listening on port 8080 — the server process is gone',
        'snip crashed the operating system',
      ],
      answer: 2,

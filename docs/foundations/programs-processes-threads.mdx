  ...
  questions={[
    {
      ...
      options: [
        'One process with two names, sharing a single block of memory',
        'Two processes, each with its own PID and private memory',
        'Two threads inside one process, sharing all their memory',
        'An error — a program file can only be running once at a time',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Threads are slower to start than processes, but otherwise the same',
        'Threads in one process share memory; processes each have their own',
        'Processes can only use one core; threads can spread across many',
        'Threads take turns on one core and never truly run at the same time',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Always 12, because each thread adds its own 1',
        'Always 11, because the second write is ignored',
        'Either 11 or 12, depending on timing',
        'Always 10, because the two writes cancel out',
      ],
      answer: 2,
    ...
    },
    {
      ...
      options: [
        'SIGTERM is faster to deliver, so the pod stops sooner',
        'SIGTERM lets it finish work and clean up; SIGKILL stops it with no cleanup',
        'SIGKILL can be caught and ignored, so it’s only a fallback',
        'There is no difference; sending both is a Kubernetes tradition',
      ],
      answer: 1,
      explain: 'SIGTERM lets the program finish in-flight work and clean up; SIGKILL stops it instantly with no cleanup. SIGTERM is a polite request, so a well-written server stops taking new requests, finishes current ones and exits cleanly. SIGKILL gives no chance to clean up, so it is the last resort.',
    },
  ]}
/>

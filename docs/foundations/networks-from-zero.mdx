...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        'TCP, because every sound must arrive, and in order',
        'UDP: a brief glitch beats waiting for lost packets to be resent',
        'DNS, because its tiny messages travel faster than anything else',
        'NAT, because it keeps the call private between the two homes',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'DNS changes need a full day before any server will accept them',
        'Resolvers cached the old answer until its TTL runs out',
        'Their browsers are still holding an open TCP connection',
        'The new server rejected them and sent them back to the old one',
      ],
      answer: 1,

...
4. **Freeze Redis** (`pause` stops the process without killing it, like a hung server), and measure again:
   ...
   ```
   **What to notice:** every redirect still returns `302`, in roughly 0.2 seconds. `/readyz` says `"ready":true` but `"degraded":true`. The logs show clicks that weren't recorded.
...
5. **Stop Redis instead** (connections refused rather than hanging), and measure again:
   ...
   ```
   **What to notice:** this is much faster than the frozen case. A dead server is easier to handle than a slow one.
  ...
  questions={[
    {
      ...
      options: [
        'Read the diff carefully until you find the bug',
        'Roll back the deploy, confirm users recover, then investigate',
    ...
    },
    {
      ...
      options: [
        'Incident commanders are usually managers who can’t debug',
        'While they’re deep in logs, nobody is coordinating, deciding, or making sure users are updated',
        'Debugging is the scribe’s job',
    ...
    },
    {
      ...
      options: [
        'Redis stores all links',
        '/readyz failed when Redis was down, so every API Pod was removed from the Service at once, even though they could serve from Postgres',
        'Kubernetes restarts Pods when Redis is down',
        'The worker crashed',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'A frozen Redis loses data',
        'Connections to a stopped server are refused immediately; a frozen one never answers, so every call waits for its full timeout',
        'Docker can’t detect a frozen container',
    ...
    },
    {
      ...
      options: [
        'Engineers should be more careful with Redis',
        'Improve reliability',
        'Add a unit test that fails if /readyz returns 503 when only Redis is down (owner: Sam, due: Friday)',
        'Consider looking into caching',
      ],
      answer: 2,

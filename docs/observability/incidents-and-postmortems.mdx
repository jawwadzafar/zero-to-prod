...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      answer: 1,
      explain: 'While they’re deep in logs, nobody is coordinating, deciding, or making sure users are updated. The IC steers. Someone else does the hands-on work, and the IC hands the role over explicitly if they need to dig in.',
    },
    {
      ...
      answer: 1,
      explain: '/readyz failed when Redis was down, so every API Pod was removed from the Service at once, even though they could serve from Postgres. Redis is a soft dependency. Readiness should only fail for hard ones (Postgres); now Redis is reported as "degraded" instead.',
    },
    {
      ...
      answer: 1,
      explain: 'Connections to a stopped server are refused immediately; a frozen one never answers, so every call waits for its full timeout. Slow is harder than dead. Timeouts are what bound the damage; with the old defaults, a single redirect could wait longer than the server’s 15-second limit.',
    },
    {
      ...
      answer: 2,
      explain: 'Add a unit test that fails if /readyz returns 503 when only Redis is down (owner: Sam, due: Friday). Specific, owned and dated, and it changes the system rather than asking people to be perfect.',
    },
  ]}
/>

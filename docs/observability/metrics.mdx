  ...
  questions={[
    {
      ...
      options: ['A gauge, set to the number of requests in the last minute', 'A counter, read with rate() for requests per second', 'A histogram, which counts requests into buckets by route', 'A summary, which calculates the rate inside the app'],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Paths contain users’ link codes, which must stay private',
        'Each label value makes a new series; paths are unbounded, routes are few',
        'Prometheus can’t store slashes in label values without escaping',
        'Routes are shorter, so each sample takes less disk space',
      ],
      answer: 1,
      explain: 'Every distinct label value creates a new time series; paths are unbounded (one per link), routes are a small fixed set. Cardinality is the main way to break a metrics system. Unbounded values belong in logs and traces.',
    },
    {
      q: 'How do you get p99 latency from snip’s metrics?',
      options: [
        'quantile(0.99, rate(snip_http_request_duration_seconds_sum[5m]) / rate(snip_http_request_duration_seconds_count[5m]))',
        'histogram_quantile(0.99, sum by (le) (rate(snip_http_request_duration_seconds_bucket[5m])))',
        'max_over_time(snip_http_request_duration_seconds_count[5m]) * 0.99',
        'Sort the duration of each request in the logs and take the 99th',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Requests, Endpoints, Dependencies: what the service talks to',
        'Rate, Errors, Duration',
        'Resources, Exhaustion, Disk: what the machine is running out of',
        'Retries, Errors, Deploys: what changed and how it went',
      ],
      answer: 1,
      explain: 'Rate, Errors, Duration: what users experience. USE (Utilisation, Saturation, Errors) is its partner for resources like CPUs, pools and queues.',
    },
    {
      q: 'increase(counter[5m]) returned 691.7 for exactly 600 requests. Why?',
      options: [
        'A bug in snip that counts some requests twice',
        'increase extrapolates to the window edges: good for trends, not exact counts',
        'Some requests were counted by two Pods, one for each replica',
        'Prometheus lost some samples and filled the gap with guesses',
      ],
      answer: 1,
      explain: 'rate and increase extrapolate to the window boundaries, giving estimates; fine for trends and alerts, not exact counts. Prometheus samples every few seconds and estimates what happened between the first and last sample and the window edges.',
    },
    {
      q: 'Why is Grafana’s snip dashboard stored as JSON files in the repository?',
      options: [
        'Grafana can’t save dashboards on its own without an external database',
        'Dashboards as code: versioned, reviewed, and identical everywhere',
        'JSON dashboards load faster than ones saved in Grafana’s database',
        'Prometheus reads the JSON to know which metrics to scrape',
      ],
      answer: 1,
      explain: 'Dashboards as code: versioned, reviewed, and recreated identically in every environment, instead of hand-made and lost. The same reasoning as infrastructure as code (chapter 12.5).',
    },
  ]}
/>

  ...
  questions={[
    {
      ...
      options: [
        'Models are too slow to read whole documents on every question',
        'Models don’t know your documents; RAG fetches passages to answer from',
        'Models forget everything between calls, so RAG stores the history',
        'Fine-tuning is too expensive, and RAG trains the model more cheaply',
      ],
      answer: 1,
      explain: 'Models don’t know your documents or recent information; RAG retrieves relevant passages and has the model answer from them. Retrieve, augment, generate: give the model the facts instead of hoping it remembers them.',
    },
    {
      q: 'Why require citations like [1] in answers?',
      options: [
        'They make answers look more trustworthy to readers',
        'Each claim traces to a passage that people and code can check',
        'The model needs them to keep track of which passage it is reading',
        'They reduce token costs by replacing long quotes with numbers',
      ],
      answer: 1,
      explain: 'Every claim can be traced to a retrieved passage, which people can verify and code can partly check. Traceability is what makes a RAG answer trustworthy. Code checks that citations exist; a judge or person checks that they support the claim.',
    },
    {
      q: 'A RAG answer is wrong. What should you look at first?',
      options: [
        'The model’s temperature, which may be adding made-up details',
        'The retrieved sources: without the right passage, no prompt will fix it',
        'The system prompt, which may not tell the model to use the sources',
        'The model version, in case the provider changed it recently',
      ],
      answer: 1,
      explain: 'The retrieved sources: if the right passage isn’t there, it’s a retrieval problem, and no prompt change will fix it. Retrieval or generation? Decide first, then fix the right half.',
    },
    {
      ...
      options: [
        ...
        'Filter retrieval by the user’s permissions before the prompt',
        'Use a bigger model, which follows access rules more reliably',
        'Check each answer afterwards and hide it if it quotes HR text',
      ],
      answer: 1,
      explain: 'Filter retrieval by the asking user’s permissions, so unauthorised passages never reach the prompt. Access control happens in retrieval, before the model. Anything in the prompt can end up in the answer.',
    },
    {
      q: 'With a 1-million-token context window, why still retrieve instead of pasting in all your documents?',
      options: [
        'Large contexts are only available on enterprise plans',
        'Every token costs time and money per question, and details get lost',
        'Models can’t actually use more than the first 100,000 tokens',
        'Retrieval makes answers more creative by mixing sources',
      ],
      answer: 1,
      explain: 'Cost and latency grow with every token on every question, and relevant details get lost in very long contexts; retrieval is cheap. Retrieve a few relevant passages: cheaper, faster and usually more accurate.',
    },
  ]}
/>

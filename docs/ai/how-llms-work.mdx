  ...
  questions={[
    {
      ...
      options: [
        'Looks the answer up in a database built from its training data',
        'Predicts the next token, appends it, and repeats',
        'Searches its training documents for the closest passage and adapts it',
        'Plans the whole answer first, then writes it out word by word',
      ],
      answer: 1,
      explain: 'Repeatedly predicts a probability for every possible next token, picks one, appends it, and repeats. Predict, pick, append, repeat. Everything else (tokens, temperature, context) is detail on that loop.',
    },
    {
      q: 'Why might an LLM struggle to count the letters in a word?',
      options: [
        'It can’t do arithmetic at all, so any counting is a guess',
        'It sees tokens like "str|aw|berry", not letters',
        'Letters are merged during training to save memory',
        'It reads each word backwards, so it loses its place',
      ],
      answer: 1,
      explain: 'It sees tokens (chunks like "str|aw|berry"), not individual letters. Tokenization hides characters from the model. Many "LLMs are dumb" examples are really tokenization.',
    },
    {
      q: 'You need a model to extract a date from an email the same way every time. Which temperature is the sensible choice?',
      options: [
        '0 (or close to it)',
        '1.0, the default, which the provider tuned for accuracy',
        'Temperature only affects length, so any value will work',
        '0.7, a balance between predictable and natural wording',
      ],
      answer: 0,
      explain: '0 (or close to it): always the most likely token, for predictable output. Low temperature for extraction and classification; higher for brainstorming. Even at 0, validate the output.',
    },
    {
      q: 'Why do LLMs sometimes state false things confidently?',
      options: [
        'They’re trained to always answer, and lying is how they avoid saying no',
        'They write the most plausible text, which isn’t necessarily true',
        'Their fact databases are out of date and never refreshed',
        'Temperature is set too high by default in most provider APIs',
      ],
      answer: 1,
      explain: 'They produce the most plausible continuation of the text, which isn’t necessarily true; nothing in next-token prediction checks facts. Hallucination follows from the training objective. Grounding (RAG), constraints and evals are the defences.',
    },
    {
      ...
      options: [
        'Web pages are too long, so the real instructions fall out of the context',
        'To the model, data and instructions are both tokens, so it may obey the page',
        'It will make the provider block your API key for misuse',
        'It isn’t dangerous, because the system prompt always wins',
      ],
      answer: 1,
      explain: 'To the model, instructions and pasted data are the same kind of thing (tokens), so it may follow instructions hidden in the data. That’s prompt injection. Chapter 14.13 covers the defences.',
    },
  ]}
/>

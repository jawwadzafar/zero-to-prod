...
- **Parallelism only helps if the server allows it.** With default settings, "4 at once" was no faster than "1 at a time": this Ollama version processed the requests one after another, and the others waited in a queue. With `OLLAMA_NUM_PARALLEL=4`, total throughput rose **by about a quarter** (51 → 63 tokens a second for the small model, 26 → 31 for the larger one). That's batching at work. How much it helps depends on the machine: on a later CI run, on a different runner, the same test roughly **doubled** throughput (38 → 79 tokens a second for the small model, 15 → 28 for the larger one), while each single request ran slower than here. With only 4 CPU cores the arithmetic soon becomes a limit too; on a GPU, batching gains are typically far larger.
...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        'Bigger models have larger vocabularies, so each token is harder to pick',
        'Every token needs all the weights read from memory, so bandwidth limits it',
        'Bigger models write longer answers, which take more time to finish',
        'The chip slows itself down to stay cool when a big model runs',
      ],
      answer: 1,
      explain: 'Every output token requires reading all the weights from memory, so speed is limited by memory bandwidth divided by model size. Decode is memory-bound. Twice the bytes to move per token means roughly half the tokens per second.',
    },
    {
      q: 'What does quantizing a model from 16-bit to 4-bit weights do?',
      options: [
        'Nothing to speed; it only shrinks the file on disk for downloading',
        'About a quarter the size and faster, with some quality loss to measure',
        'Makes it more accurate, since rounding removes noise from the weights',
        'Removes three quarters of the parameters, keeping the most important',
      ],
      answer: 1,
      explain: 'Makes it about a quarter of the size and faster to decode, with some quality loss that you should measure on your own task. Same number of parameters, fewer bits each. Smaller and faster, slightly worse, unevenly so: evaluate the exact file you serve.',
    },
    {
      ...
      options: [
        'Throughput, in tokens per second across all users',
        'Time to first token',
        'The model’s parameter count, since bigger models start slower',
        'Tokens per second per user, once the answer is streaming',
      ],
      answer: 1,
      explain: 'Time to first token: queueing, model loading and prefill before anything appears. Perceived speed is dominated by the wait before the first token when responses stream.',
    },
    {
      q: 'Why can a GPU server handle many users at once far more efficiently than one?',
      options: [
        'It runs a separate copy of the model per user, all at the same time',
        'Batching: one pass over the weights moves many requests on a token each',
        'Users with similar questions share one answer from a cache',
        'It skips the prefill phase for users after the first',
      ],
      answer: 1,
      explain: 'Batching: one pass over the weights advances many requests by a token each, so throughput rises until memory (mostly the KV cache) runs out. The expensive part, moving the weights, is shared across the batch. The KV cache per active request is what limits the batch size.',
    },
    {
      q: 'Which is the strongest reason to self-host a model rather than use a hosted API?',
      options: [
        'Open models are free to download, so it always costs less overall',
        'The data can’t legally leave, or high steady volume a small model handles',
        'Self-hosted models are always better at your own specific tasks',
        'Hosted APIs can’t stream tokens back as they are generated',
      ],
      answer: 1,
      explain: 'The data legally can’t leave your infrastructure, or you have high, steady volume of a task a small model passes your evals on. Hardware, operations and idle time cost money. Self-hosting wins on data control, offline use, or steady high volume.',
    },
  ]}
/>

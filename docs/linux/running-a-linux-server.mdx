  ...
  questions={[
    {
      ...
      options: [
        'sudo systemctl restart snip, so the new unit file is loaded',
        'sudo systemctl enable snip — start runs it now; enable runs it at boot',
        'To install snip with apt, so it is registered as a system package',
        'sudo systemctl daemon-reload, so systemd remembers the unit',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'The firewall rules, in case the port is being blocked',
        'journalctl -u <service>: its output plus systemd’s notes on each crash',
        'crontab -l, in case a scheduled job keeps stopping it',
        'The package list, in case an update replaced the binary',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'df caches its numbers, so it will catch up in a few minutes',
        'A running process still has the file open, so its space isn’t freed yet',
        'Deleted files go to a recycle bin until you empty it with rm -rf',
        'The file was a symbolic link, so only the link was removed',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'cron can only run single commands, not whole scripts',
        'cron runs jobs with a minimal PATH, so commands need absolute paths',
        'The job runs before the disks are mounted in the morning',
        'cron requires root, and your script runs as your own user',
      ],
      answer: 1,

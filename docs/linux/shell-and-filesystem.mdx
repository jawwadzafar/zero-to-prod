  ...
  questions={[
    {
      ...
      options: [
        'The internet is down, or the kubectl download was corrupted',
        'Its folder isn’t in $PATH, or this terminal started before $PATH changed',
        'You need to restart the computer so the program gets registered',
        'kubectl only runs with sudo until it has been configured',
      ],
      answer: 1,
      explain: 'It is installed in a folder that is not in $PATH, or this terminal started before $PATH was updated. The shell finds commands by searching $PATH. Either the install location is not listed there, or your terminal loaded $PATH before the change. Check with which, fix $PATH, or open a new terminal.',
    },
    {
      q: 'You run ls *.log in a folder with a.log and b.log. What does the ls program actually receive?',
      options: [
        'The text *.log, which ls then matches against the folder',
        'The arguments a.log and b.log — the shell expanded the * first',
        'Nothing; ls reads the folder itself and filters the names',
        'An error, because * must be quoted to reach a program',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'The script is too long for Linux to load from another folder',
        'It uses relative paths, resolved from the current working directory',
        'Linux only allows running scripts from inside their own folder',
        'The file is hidden from scripts started in other folders',
      ],
      answer: 1,

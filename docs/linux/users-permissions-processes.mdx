  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        ...
        'Give www-data (or its group) read permission on the files, and x on the folders leading to them',
        'Delete and recreate the files',
      ],
      answer: 2,
      explain: 'Grant exactly the access needed to exactly the user that needs it. 777 and running as root both "work" by removing protection, which turns any bug into a security hole.',
    },
    {
      q: 'You set API_URL=http://localhost:8080 in your shell, then run your program, but it says API_URL is empty. What happened?',
      options: [
        'Environment variables only work on servers',
        'The variable was not exported, so the child process did not inherit it',
        'Your program needs sudo',
        'URLs cannot be stored in variables',
      ],
      answer: 1,

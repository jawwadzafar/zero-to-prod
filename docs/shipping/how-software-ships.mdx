  ...
  questions={[
    {
      ...
      options: [
        'They are the same practice, named by two different books',
        'Delivery: every change could ship, a human decides; deployment: all ship',
        'Deployment covers servers; delivery covers apps sent to phones',
        'Delivery ships every change at once; deployment waits for approval',
      ],
      answer: 1,
      explain: 'Delivery makes every passing change releasable and a human decides when to release; deployment releases every passing change to production automatically. Both automate everything up to production. The difference is whether the final step is automatic.',
    },
    {
      q: 'Why build the artefact once and promote it through environments, instead of rebuilding for each?',
      options: [
        'To save disk space in the registry, since images are large',
        'A rebuild can differ from what was tested, so production runs untested code',
        'Registries only allow one push per version number',
        'Rebuilding is slower, and that delays every deploy',
      ],
      answer: 1,
      explain: 'A rebuild could differ from what was tested (newer base image or dependency), so production would run something never tested. Same bytes everywhere. What changes between environments is configuration.',
    },
    {
      q: 'A version goes from 2.3.1 to 3.0.0. What does SemVer tell users?',
      options: [
        'Only bug fixes, safe to upgrade to without changes',
        'A breaking change: users may need to change things to upgrade',
        'New features that don’t change anything existing',
        'Nothing; the major number is marketing, not a promise',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'You must trade one for the other: faster teams break more',
        'They go together: teams shipping small changes often fail less',
        'Stability matters more than speed for most users',
        'Only large companies with dedicated release teams have both',
      ],
      answer: 1,
      explain: 'They go together: teams that ship small changes often also have lower failure rates and faster recovery. Small changes are easier to review, test and undo. Big releases bundle risk.',
    },
    {
      q: 'Which of these did this repository’s CI catch that static checks alone would have missed?',
      options: [
        'A spelling mistake in a chapter, caught by the docs linter',
        'Pods that never started: runAsNonRoot couldn’t check a named user',
        'A Go file that failed to compile after a merge conflict',
        'A YAML indentation error in a Kubernetes manifest',
      ],
      answer: 1,
      explain: 'Kubernetes Pods that could never start because runAsNonRoot couldn’t be verified for a named image user. The manifests were valid YAML and passed schema checks. Only running them on a real cluster revealed the problem.',
    },
    {
      q: 'Your team’s CI takes 70 minutes and people have started merging without waiting. What would you do first?',
      options: [
        'Remove the slowest tests, since most failures are caught early anyway',
        'Make checks fast (parallel, cached), then require them before merging',
        'Add more tests to the same job, so fewer bugs reach main in between',
        'Ask people to be patient, and review any merge that skipped the checks',
      ],
      answer: 1,
      explain: 'Make the main checks fast (parallel jobs, caching, moving slow suites to scheduled runs) so waiting is reasonable, and require checks to pass before merging. A pipeline only protects you if people wait for it. Speed is a feature of a pipeline.',
    },
  ]}
/>

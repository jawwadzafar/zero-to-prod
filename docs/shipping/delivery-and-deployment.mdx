  ...
  questions={[
    {
      ...
      options: [
        'There is none',
        'Deploying puts new code on production machines; releasing lets users experience the change. Canaries and feature flags separate the two',
        'Releasing is for mobile apps only',
        'Deploying happens after releasing',
      ],
      answer: 1,
      explain: 'Separating them turns deployments into routine events and releases into controlled decisions.',
    },
    {
      q: 'A rolling update finishes, all Pods are ready, and users report wrong results. Why didn’t the rollout stop?',
      options: [
        'Rolling updates never check anything',
        'Readiness only checks that Pods respond, not that they’re correct; catching wrong behaviour needs metrics-based checks, as in canary analysis',
        'The rollout was too fast',
        'Kubernetes ignores readiness during rollouts',
      ],
      answer: 1,
      explain: 'Probes answer "is it up?". Business metrics and error rates answer "is it right?".',
    },
    {
      q: 'What is the main advantage of a canary over blue-green?',
      options: [
        'Canaries need no extra capacity at all',
        'Only a small share of real users meet the new version at first, limiting the blast radius while testing with real traffic',
        'Canaries switch everyone at once',
        'Canaries avoid database changes',
      ],
      answer: 1,
      explain: 'Blue-green switches everyone together (with instant switch-back). Canaries expose a fraction first.',
    },
    {
      q: 'Why can’t a release that renamed a database column simply be rolled back?',
      options: [
        'Rollbacks are forbidden',
        'The old code expects the old column; code rolls back but the schema change remains. Use expand-and-contract so every step works with both versions',
        'Databases can’t be changed',
        'The column is cached',
      ],
      answer: 1,
      explain: 'During any rollout old and new versions share one database, so schema changes must be backwards compatible.',
    },
    {
      q: 'What does a progressive delivery controller like Argo Rollouts add to a canary?',
      options: [
        'A new database',
        'Automatic step-by-step traffic shifting with metric checks, promoting on good numbers and rolling back on bad ones',
        'Faster builds',
        'Feature flags',
      ],
      answer: 1,
      explain: 'It replaces a human watching dashboards with an automated, consistent decision.',
    },
    {

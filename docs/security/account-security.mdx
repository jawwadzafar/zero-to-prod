  ...
  questions={[
    {
      ...
      options: [
        'Five is too many',
        'It lets an attacker lock any user out on purpose; growing delays, rate limits and MFA stop guessing without handing attackers a denial-of-service button',
        'Lockouts are illegal',
        'Attackers never fail five times',
      ],
      answer: 1,
      explain: 'A defence that can be triggered by the attacker against the victim is a new attack.',
    },
    {
      q: 'How does the Pwned Passwords range API check a password without learning it?',
      options: [
        'It encrypts the password first',
        'Only the first 5 characters of the SHA-1 hash are sent; the service returns all leaked hashes with that prefix, and the comparison happens locally',
        'It uses a VPN',
        'It stores the password briefly and deletes it',
      ],
      answer: 1,
      explain: 'k-anonymity: hundreds of different hashes share any prefix, so the prefix reveals almost nothing.',
    },
    {
      q: 'Why must a new session ID be issued at login?',
      options: [
        'Session IDs wear out',
        'Otherwise an attacker who planted a known ID in the victim’s browser before login would share the logged-in session (session fixation)',
        'To make cookies smaller',
        'Browsers require it',
      ],
      answer: 1,
      explain: 'The privilege level changed, so the identifier must change with it.',
    },
    {
      q: 'An already-used refresh token is presented again. What should the server conclude and do?',
      options: [
        'It’s a harmless retry; accept it',
        'Two parties have the token, so it was probably stolen: revoke the whole token family and require a fresh login',
        'Issue two new tokens',
        'Ignore it silently',
      ],
      answer: 1,
      explain: 'Rotation with reuse detection turns token theft into a detectable event.',
    },
    {
      q: 'A reset email’s link is built from the request’s Host header. What can an attacker do?',
      options: [
        'Nothing; the email goes to the real owner',
        'Request a reset for the victim with a forged Host, so the genuine email contains a link to the attacker’s domain with a valid token',
        'Read the victim’s inbox',
        'Change the victim’s password directly',
      ],
      answer: 1,
      explain: 'Build absolute URLs from configuration, never from request headers.',
    },
  ]}
/>

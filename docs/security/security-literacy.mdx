...
A **Certificate Authority** is an organisation trusted to check that whoever requests a certificate for `snip.example` really controls `snip.example`. **Let's Encrypt** issues free certificates automatically, and today most of the web uses it or a similar service. Certificates expire (Let's Encrypt's after 90 days), so renewal must be **automated** — tools like `certbot`, or Kubernetes' **cert-manager** (chapter 10.3), handle it.

...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      answer: 1,
      explain: 'Many request goroutines use the same map at once, and unsynchronised writes cause races or crashes. Go maps are not safe for concurrent writes. The (RW)mutex makes each read-modify-write happen one at a time, preventing lost updates and "concurrent map writes" crashes.',
    },
    {

  ...
  questions={[
    {
      ...
      options: [
        'A list of the lines you changed since the previous commit',
        'A snapshot of the whole project, plus message, author, time and parent',
        'A backup copy of each file you changed, stored separately',
        'An upload of your changes to GitHub, marked with your name',
      ],
      answer: 1,
      explain: 'A snapshot of the whole project, with a message, author, time and a link to the previous commit. Commits are snapshots linked to their parents, forming history. Diffs are computed by comparing snapshots. Uploading to GitHub is a separate step (push).',
    },
    {
    ...
    },
    {
      ...
      options: [
        'git commit --amend, then push so everyone gets the fix',
        'git revert <hash>: a new commit undoing it, shared history untouched',
        'git reset --hard to the commit before it, then force-push',
        'git restore . to put every file back as it was before',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Yes — the latest version no longer contains it, and that is what people clone',
        'No — it’s still in history and may be scraped already; revoke it and make a new one',
        'Yes, if nobody has starred or forked the repository yet',
        'Only if you also squash the two commits into one',
      ],
      answer: 1,

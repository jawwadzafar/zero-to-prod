...
=======
	if c.RateLimit, err = strconv.Atoi(env("SNIP_RATE_LIMIT", "100")); err != nil || c.RateLimit < 1 {
...
:::danger
The last command deletes the practice directory with `rm -rf`, which asks no questions and has no undo. Check the path is exactly `~/branch-lab` before you press Enter.
:::
  ...
  questions={[
    {
      ...
      options: [
        'A full copy of the project folder, kept inside .git',
        'A named, movable label pointing at a commit',
        'A separate repository that shares history with main',
        'A frozen backup of main taken when you create it',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Whenever both branches changed the same file at all',
        'When both branches changed the same lines differently',
        'Whenever the merge needs a merge commit to join them',
        'When the branches were created by different people',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'git push --force, since your commits are the newer ones',
        'git pull to merge their commits in, resolve conflicts, then push',
        'Delete your local branch, clone again and redo your work',
        'Wait until the remote finishes processing, then push again',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Rebase is slower than merge on history others have pulled',
        'Rebase makes new commits with new hashes, so others’ history won’t match',
        'Rebase deletes the original branch once it has finished',
        'Rebase only works on commits nobody has pushed yet, and fails',
      ],
      answer: 1,

...
1. **Exponential backoff** — wait longer after each failure: 50 ms, 100 ms, 200 ms, 400 ms… giving the dependency room to recover. snip's click worker does exactly this when processing fails, doubling its wait up to 30 seconds (`cmd/snip-worker/main.go`):
   ```go

...
2. **Watch round robin.** Create a key and a link through nginx, then follow it several times while watching both API copies' logs:
   ...
   curl -s -X POST localhost:8080/api/links -H "Authorization: Bearer $KEY" -d '{"url":"https://go.dev","slug":"lb"}' >/dev/null
   for i in $(seq 1 6); do curl -s -o /dev/null localhost:8080/lb; done
...
3. **Kill a copy; watch the proxy route around it.**
   ...
   for i in $(seq 1 4); do curl -s -o /dev/null -w "%{http_code} " localhost:8080/lb; done; echo   # still 302s

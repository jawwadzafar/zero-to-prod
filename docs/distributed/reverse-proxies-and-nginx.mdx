  ...
  questions={[
    {
      ...
      options: [
        'The users’ browsers, in case a stale cache is being served',
        'The backends behind the proxy: running, and on the right port?',
        'The DNS records, in case the domain points at the wrong proxy',
        'The proxy’s TLS certificate, which may have expired overnight',
      ],
      answer: 1,
      explain: 'The backends behind the proxy: are they running, crashing, or listening on the configured port and address?. 502 is the proxy saying it could not get a valid response from any backend. 504 would point at slowness instead.',
    },
    {
      q: 'Why can snip use simple round-robin load balancing without sticky sessions?',
      options: [
        'Because nginx doesn’t support sticky sessions without a paid module',
        'snip’s copies are stateless, so any copy can answer any request',
        'Because snip’s requests are all the same size, so load evens out',
        'Because round robin is always the fairest way to spread load',
      ],
      answer: 1,
      explain: 'Because snip’s copies are stateless — all state is in Postgres and Redis — so any copy can answer any request. Statelessness (chapter 1.5) lets requests go to any copy, which makes scaling and failover simple.',
    },
    {
      q: 'Your app rate-limits by the first IP in X-Forwarded-For. What is the problem?',
      options: [
        'Nothing; the first address is the one the proxy saw, so it’s reliable',
        'Clients can set that header themselves and pretend to be any IP',
        'X-Forwarded-For only ever holds the proxy’s own IP address',
        'Browsers strip X-Forwarded-For, so the limit never sees an address',
      ],
      answer: 1,
      explain: 'Clients can send their own X-Forwarded-For header, so they can pretend to be any IP and bypass the limit. Only entries appended by proxies you control are trustworthy. Read from the right, or have the trusted proxy set the client address.',
    },
    {
      q: 'What is TLS termination at the reverse proxy?',
      options: [
        'Turning off encryption for everyone once they reach the proxy',
        'The proxy handles HTTPS with clients and forwards over the private network',
        'Ending TLS sessions early, so each client reconnects often',
        'Checking each backend’s certificate before forwarding traffic',
      ],
      answer: 1,
      explain: 'The proxy holds the certificate, handles HTTPS with clients, and forwards traffic to backends over the private network. Certificates and TLS configuration live in one place, and backends can speak plain HTTP inside a trusted network (or re-encrypt where required).',
    },
  ]}
/>

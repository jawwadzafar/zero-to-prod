...
//    claimIdle (a minute): take them over.
if len(msgs) == 0 {
...
// 3. New events (">"), waiting up to 2s for some to arrive.
if len(msgs) == 0 {
	msgs, err = c.read(ctx, ">")
}
...
- **Re-claiming pending events.** On each pass, the worker first retries its **own** pending entries (start ID `"0"`): ones it received but never acknowledged, because processing failed. Then it takes over **other workers'** pending entries that have sat unacknowledged for over a minute (`XAUTOCLAIM`). Then it reads new ones (`">"`).

That middle step was missing for a long time, and the bug it fixes is worth understanding. Each worker's name includes its process ID, so it's unique. When a worker crashed mid-batch, the replacement (a new process, or a new pod in Kubernetes) had a **different** name, so it never asked for the dead worker's pending entries, and nobody else did either. Those clicks were never counted, and sat in Redis's pending list forever. The comment in the code said crashes caused "a small over-count"; in fact, the crash case caused a permanent *under*-count. A test now replays it: a worker takes three events and "dies" without acknowledging them, and a differently named worker must count all three once they've been idle long enough. When you rely on a queue's redelivery, check who actually redelivers.
  ...
  questions={[
    {
      ...
      options: [
        'Postgres can’t update a row while redirects are reading it',
        'Instant redirects, absorbed bursts, and a DB blip can’t break them',
        'Queues make counts exact, where direct updates can drop some',
        'Redis is the source of truth for clicks, and Postgres is a copy',
      ],
      answer: 1,
      explain: 'The visitor gets an instant redirect, bursts are absorbed, and a database blip doesn’t break redirects. Slow, non-urgent work moves off the request path. The queue buffers bursts and outages; workers catch up at their own pace.',
    },
    {
      q: 'Why does the worker acknowledge (XACK) events only after writing the counts?',
      options: [
        'Redis requires the write to finish before an ack is accepted',
        'So a crash before the write gets them redelivered, not lost',
        'To make the worker faster, by batching the acks with the writes',
        'Acking first would count every click twice, once per step',
      ],
      answer: 1,
      explain: 'So a crash before the write means the events are redelivered instead of lost (at-least-once). Ack after the work, and a crash leads to redelivery (possible duplicates) rather than loss. Ack before the work gives at-most-once: losses on crashes.',
    },
    {
      q: 'What is the outbox pattern for?',
      options: [
        'Sending emails and notifications in batches, to save money',
        'Committing a change and its event together: write both in one transaction',
        'Moving failed messages aside so they don’t block the queue',
        'Keeping a copy of every sent message in case the broker loses it',
      ],
      answer: 1,
      explain: 'Making a database change and its event commit together, by writing the event to an outbox table in the same transaction and publishing it later. It solves the dual-write problem: two systems cannot share a transaction, but a table in the same database can.',
    },
    {
      q: 'A queue’s depth keeps growing for hours. What does it tell you?',
      options: [
        'Everything is healthy; queues are meant to hold work for later',
        'Consumers can’t keep up: add workers, speed them up, or shed load',
        'The broker is delivering too fast for the network to carry',
        'Messages are being duplicated, so the count overstates the work',
      ],
      answer: 1,
      explain: 'Consumers can’t keep up with producers — add workers, speed up processing, or shed load, before the queue’s limit or memory is reached. Short growth absorbs bursts; sustained growth is backpressure you must act on. Monitor depth and oldest-message age.',
    },
  ]}
/>

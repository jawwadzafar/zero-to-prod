  ...
  questions={[
    {
      ...
      options: [
        'It’s faster than a queue',
        'Writing to the database and then to a queue separately can lose the event if the process crashes in between; one transaction makes the link and its event succeed or fail together',
        'Queues can’t store JSON',
        'To avoid needing a worker',
      ],
      answer: 1,
      explain: 'The dual-write problem: two separate writes to two systems can’t both be guaranteed without something like an outbox.',
    },
    {
      q: 'Webhooks are delivered at least once. What must customers do as a result?',
      options: [
        'Nothing',
        'Deduplicate by event ID, because a timed-out delivery that actually succeeded will be retried',
        'Respond faster than 1 ms',
        'Poll for events instead',
      ],
      answer: 1,
      explain: 'Exactly-once delivery over a network isn’t possible; idempotent handling makes duplicates harmless.',
    },
    {
      q: 'In the shared rate limiter, why must "check and increment" be one atomic operation?',
      options: [
        'It’s required by Redis',
        'Separately, two concurrent requests can both read the same count, both be allowed, and exceed the limit: a race condition',
        'It uses less memory',
        'It makes the limiter fail closed',
      ],
      answer: 1,
      explain: 'The classic read-modify-write race from chapter 6.5, across processes instead of threads.',
    },
    {
      q: 'In the company document assistant, where must permission filtering happen?',
      options: [
        'In the system prompt',
        'Inside the search query, before ranking and before anything reaches the model',
    ...
    },
    {
      ...
      options: [
        'Asking about scale first',
        'Naming technologies without explaining what each piece does, why it’s needed, or what happens when it fails',
        'Drawing a simple first design',
        'Changing their mind after a good hint',
      ],
      answer: 1,
      explain: 'Interviewers judge reasoning. "We’ll use Kafka" says nothing; "we need a durable queue because…" says a lot.',
    },
  ]}
/>

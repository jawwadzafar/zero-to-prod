  ...
  questions={[
    {
      ...
      options: [
        'A library you import to build APIs that follow the open standard',
        'A machine-readable description of an API: endpoints, bodies, errors',
        'A list of API keys and which endpoints each is allowed to call',
        'A type of database that stores API requests for later replay',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'For public APIs that browsers and curl call directly',
        'For fast, strongly typed calls between your own services',
        'When you want to change the API without a contract getting in the way',
        'When the API has one endpoint, so a whole REST design isn’t needed',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Someone compares it with the code by hand before each release',
        'CI lints it, and a Go test fails on any error code it doesn’t document',
        'The contract is generated from the database schema on every build',
        'It doesn’t; drift is unavoidable, so clients read the code instead',
      ],
      answer: 1,
      explain: 'CI lints the contract and a Go test fails if code sends an error code the contract does not document. Automated checks make drift fail loudly on every push, so it is fixed while the change is fresh rather than discovered by a broken client.',
    },
  ]}
/>

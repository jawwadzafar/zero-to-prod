  ...
  questions={[
    {
      ...
      options: [
        'No — the app’s checks already run before any request is sent',
        'Yes — anyone can call the API directly, so only server checks count',
        'Only for admin users, since they bypass the app’s checks',
        'Only in production, where real users send real data',
      ],
      answer: 1,
      explain: 'Yes — anyone can call the API directly, so server-side checks are the only ones that count. Client-side checks improve the user experience but can be bypassed with curl or a script. The server is the only real line of defence.',
    },
    {
      q: 'Why does snip use json.Decoder with DisallowUnknownFields?',
      options: [
        'It makes decoding faster by skipping fields it does not know',
        'So a typo like {"link": ...} is rejected, not silently ignored',
        'JSON requires it whenever a struct has tags on its fields',
        'To hide unknown fields from the logs, in case they hold secrets',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Allow-lists are shorter to type than a list of every bad character',
        'You can’t list every dangerous input, but you can describe what is valid',
        'Go’s regular expressions can only match allowed characters',
        'Allow-lists run faster, because they stop at the first match',
      ],
      answer: 1,

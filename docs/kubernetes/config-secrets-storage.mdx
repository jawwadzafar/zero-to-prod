  ...
  questions={[
    {
      ...
      answer: 1,
      explain: 'Only after they restart, e.g. via kubectl rollout restart, because environment variables are read at start. Mounted files update eventually, but env vars never change in a running process. Roll out the change.',
    },
    {
      ...
      answer: 1,
      explain: 'Nothing: it is a reversible encoding. Protection comes from RBAC, encryption at rest and keeping values out of Git. base64 -d reverses it instantly. Treat a Secret in YAML exactly like a password in plain text.',
    },
    {
      ...
      answer: 1,
      explain: 'By storing values in a secret manager and syncing them in (External Secrets), or committing only encrypted values (Sealed Secrets, SOPS). A private repository still exposes the secret to everyone with access, forever, in history. Keep the value somewhere built for secrets.',
    },
    {
      ...
      answer: 1,
      explain: 'A claim requests storage; the StorageClass’s provisioner creates a matching PersistentVolume on demand and binds it to the claim. Apps ask (PVC); the platform decides how to satisfy it (StorageClass → PV).',
    },
    {
      ...
      answer: 1,
      explain: 'The StatefulSet recreates postgres-0, which mounts the same PersistentVolumeClaim, data-postgres-0. Stable identity plus a per-Pod claim means the same Pod name always gets the same disk.',
    },
    {
      ...
      answer: 1,
      explain: 'No: it’s the live disk. Corruption, mistakes and deletions affect it directly. Databases still need tested backups. Durable storage keeps data across restarts. Backups keep it across mistakes and disasters.',
    },
  ]}
/>

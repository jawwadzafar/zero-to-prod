...
Larger platforms add a **service mesh** (Istio, Linkerd, Cilium's mesh). Sidecar or node proxies give every connection mutual TLS (chapter 7.1), retries, per-request load balancing (fixing the gRPC problem above) and detailed traffic metrics, without changing the application. It's powerful and adds real operational weight. Most teams start with Services, a Gateway and NetworkPolicies, and add a mesh only when they need what it offers.
:::
...
3. **Install a Gateway implementation** (Envoy Gateway, with Helm; chapter 10.6 explains Helm) and snip's Gateway and route:
   ...
   kubectl -n snip wait --for=condition=Accepted gateway/snip --timeout=120s
   kubectl -n envoy-gateway-system wait --for=condition=Ready pod -l gateway.envoyproxy.io/owning-gateway-name=snip --timeout=120s
   kubectl -n snip get gateway snip          # PROGRAMMED False on kind: no load balancer to give it an address
   ```
   On a cloud cluster the Gateway would become **Programmed**, with a public address from a cloud load balancer. kind has no load balancer, so the Gateway is accepted and its proxy runs, but it never gets an external address. Envoy Gateway runs the actual proxy as a Service in its own namespace. Forward a port to it and send a request with the route's hostname:
   ```bash
  ...
  questions={[
    {
      ...
      options: [
        'Pod IPs are private to the node and can’t be reached from others',
        'Pods are replaced with new IPs; a Service gives a stable name in front',
        'Pod IPs are IPv6 only, which many clients can’t connect to',
        'Pods share their node’s IP, so the address isn’t unique to them',
      ],
      answer: 1,
      explain: 'Pods are replaced with new IPs all the time; a Service provides a stable address and name in front of whichever Pods are ready. Stable names and virtual IPs (Services) plus DNS are Kubernetes’ answer to constantly changing Pods.',
    },
    {
      q: 'How does a failing readiness probe remove a Pod from a Service?',
      options: [
        'The kubelet restarts the Pod, so it comes back with a fresh IP address that no Service knows yet',
        'It’s dropped from the Service’s EndpointSlices, so no new traffic goes to it',
        'The cluster DNS stops returning the Pod’s name until its probe passes again',
        'The Pod is restarted, which takes it out of rotation',
      ],
      answer: 1,
      explain: 'The Pod is dropped from the Service’s EndpointSlices, so kube-proxy stops routing new connections to it. Only ready Pods appear in endpoints. The Pod keeps running, just without new traffic.',
    },
    {
      q: 'A Pod in the monitoring namespace connects to "api:80" and it fails to resolve. Why?',
      options: [
        'DNS is disabled across namespaces unless a policy allows it',
        'Short names resolve in the caller’s namespace; use api.snip instead',
        'Services can only be reached from other namespaces by IP',
        'Port 80 is reserved for Ingress, so Services use another port',
      ],
      answer: 1,
      explain: 'Short names resolve within the caller’s own namespace; from another namespace use api.snip or api.snip.svc.cluster.local. Search domains start with the Pod’s own namespace. Add the namespace to the name.',
    },
    {
      q: 'What does the Gateway API improve over Ingress?',
      options: [
        'It is faster, because routes are compiled into the kernel',
        'Portable, richer routing and separate roles, instead of controller annotations',
        'It removes the need for Services, because Routes send traffic straight to Pod IPs',
        'It adds TLS, which Ingress has no way to configure',
      ],
      answer: 1,
      explain: 'Standard, portable objects for richer routing (headers, weights, redirects) and role separation (GatewayClass, Gateway, Routes), instead of controller-specific annotations. The same HTTPRoute works across implementations. Ingress features beyond host and path lived in annotations that differ per controller.',
    },
    {
      q: 'You add a default-deny NetworkPolicy to a namespace. What else must you add for the app to keep working?',
      options: [
        'Nothing; traffic inside a namespace is always still allowed',
        'Allow rules for each needed connection, such as api to postgres',
        'A LoadBalancer Service, which bypasses NetworkPolicies',
        'A new namespace for the database, outside the default-deny',
      ],
      answer: 1,
      explain: 'Policies allowing each needed connection, e.g. api and worker to postgres and redis, and traffic to the API’s port. Deny by default, then allow what’s needed. In the lab, snip still worked while an unrelated Pod couldn’t reach Postgres.',
    },
    {
      q: 'Your NetworkPolicies are applied but traffic is not being blocked. What is a likely cause?',
      options: [
        'Policies only apply to connections made after the next restart',
        'The cluster’s CNI plugin doesn’t enforce them, so they’re ignored',
        'Policies only apply to traffic that goes through a Service',
        'kubectl apply stored them, but they need a namespace restart',
      ],
      answer: 1,
...
1. The browser resolves `snip.example.com` via public DNS to the **cloud load balancer's** public IP (DNS, chapter 1.4), which was created for the Gateway.

...
| **readinessProbe** | Should it receive traffic right now? | **Take it out of the Service**, but don't restart it | `/readyz`: fails if Postgres is down (Redis down = degraded, still ready; chapter 13.5) |
  ...
  questions={[
    {
      ...
      options: [
        'Pods cannot be created from YAML, only by other controllers',
        'A bare Pod isn’t replaced if it dies; a controller replaces its Pods',
        'Pods are reserved for system components in kube-system',
        'Pods created directly skip the scheduler, so they all land on the control-plane node',
      ],
      answer: 1,
      explain: 'Pods are disposable and aren’t recreated if they die; a Deployment, StatefulSet or Job manages them and replaces them. A bare Pod that dies stays dead. Controllers keep the desired number of Pods running.',
    },
    {
      q: 'What connects a Deployment or a Service to its Pods?',
      options: [
        'The Pods’ IP addresses, recorded when each Pod starts',
        'Labels on the Pods, matched by a selector',
        'The owner name written into each Pod by the scheduler',
        'The container names, which must match the Deployment’s name',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Liveness probes cannot make network calls to other services',
        'A DB outage would then restart every API Pod; readiness only pauses traffic',
        'Readiness probes run far less often, so checking Postgres there puts less load on it',
        'It is a mistake; both should check every dependency the API needs',
      ],
      answer: 1,
      explain: 'If liveness checked the database, a database outage would restart every API Pod repeatedly; readiness just stops sending them traffic until the database recovers. Liveness failure means "restart me". Readiness failure means "don’t send me traffic right now".',
    },
    {
      q: 'With replicas: 2, maxSurge: 1 and maxUnavailable: 0, what happens if the new version never becomes ready?',
      options: [
        'Both old Pods are replaced anyway once the deadline passes',
        'It stalls: one new Pod isn’t ready, and both old Pods keep serving',
        'One old Pod is removed to make room, so capacity drops to one',
        'Kubernetes rolls back to the previous version automatically',
      ],
      answer: 1,
      explain: 'The rollout stalls: one new Pod exists but isn’t ready, and the two old Pods keep serving. Old Pods are only removed when new ones are ready. Fix forward, or kubectl rollout undo.',
    },
    {
      q: 'A container uses more memory than its limit. What happens?',
      options: [
        'It is slowed down until its usage falls below the limit again',
        'It is OOM-killed (exit 137) and restarted',
        'Kubernetes moves it to a node with more free memory',
        'Nothing; limits are only used for scheduling decisions',
      ],
      answer: 1,
      explain: 'The kernel OOM-kills it (exit code 137) and the kubelet restarts it. Memory can’t be throttled, so exceeding the limit kills the container. CPU over its limit is throttled instead.',
    },
    {
...
2. Check what the readiness probe checks. For snip, `/readyz` fails when it can't reach Postgres (it also reports Redis, but a Redis outage alone doesn't fail it), so check the Postgres Pod is Running and ready, and that its Service exists: `kubectl get pods,svc`.
...
4. Check the configuration the Pod actually received: `kubectl exec deploy/api -- env` isn't possible in distroless, but `kubectl get pod NAME -o yaml` shows the env sources, and `kubectl get cm snip-config -o yaml` shows values. A typo in `SNIP_DATABASE_URL` is a classic cause.

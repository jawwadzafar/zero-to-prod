...
Here it is in snip's real code. The business logic, in `internal/links/service.go` (with the tracing lines from chapter 13.3 left out):
...
func (s *Service) Resolve(ctx context.Context, slug string) (ResolveResult, error) {
	cacheBroken := false
	if s.cache != nil {
		u, ok, err := s.cache.GetURL(ctx, slug)
		if err != nil {
			cacheBroken = true
		}
		if err == nil && ok {
...
(`SetNX` means "set only if the key doesn't exist yet". The reason is a bug we found later, explained below in "Even the right order has a gap".)
...
### Even the right order has a gap

For a long time, that was all snip did, and it has a race that only a careful review found. Picture two things happening at almost the same moment:

```mermaid
sequenceDiagram
    participant R as Redirect request
    participant O as Owner's delete
    participant P as Postgres
    participant C as Redis cache
    R->>C: GET slug → miss
    R->>P: SELECT slug → found (not deleted yet)
    O->>P: DELETE slug
    O->>C: DEL slug (nothing there yet)
    R->>C: SET slug = url, for 1 hour
    Note over C: the deleted link redirects again, for an hour
```

The redirect read the link a moment before it was deleted, and wrote it to the cache a moment after the eviction. For the next hour, a link its owner deleted, perhaps because it turned out to point at a phishing page, keeps redirecting.

The fix uses two small changes in `internal/cache/redis.go`:
...
- **Delete leaves a tombstone**: a marker (an empty value, kept for a minute) instead of an empty key. `GetURL` treats a tombstone as a miss.
- **Filling the cache only writes to an empty key** (`SET NX`). The late write from the redirect finds the tombstone and does nothing.

snip's links never change after they're created, so "only fill an empty key" loses nothing. The test, `TestLateFillCannotResurrectADeletedLink`, replays the race: delete, then a late fill, then a lookup, which must miss. It fails on the old code and passes on the new.

The general lesson: whenever a cache is filled by readers and invalidated by writers, ask what happens when a slow read finishes after a fast write. Tombstones, version numbers, and "only fill if absent" are the usual answers.
  ...
  questions={[
    {
      ...
      options: [
        'The request fails and the client retries once the cache is warm',
        'The app reads the database, answers, and caches it for next time',
        'Redis fetches the row from Postgres by itself and returns it',
        'The whole cache is cleared, since it’s clearly out of date',
      ],
      answer: 1,
      explain: 'The application reads the database, returns the answer, and stores it in the cache for next time. The application owns the logic: try the cache, fall back to the source of truth, and fill the cache on the way out.',
    },
    {
      q: 'Why does snip delete from Postgres first and only then evict the cache entry?',
      options: [
        'Redis is slower than Postgres, so it should go last',
        'Evicting first lets a racing request re-cache the old row from the database',
        'Postgres locks the row until the cache has been told about it',
        'It doesn’t matter; both finish within a millisecond anyway',
      ],
      answer: 1,
      explain: 'If the cache were cleared first, a racing request could re-read the old row from the database and re-cache it. Evicting first leaves a window where the old value is still in the database. Change the truth first, then remove the copy.',
    },
    {
      q: 'A popular key expires and thousands of requests hit the database at once. What is this called?',
      options: [
        'A deadlock between the cache and the database',
        'A cache stampede',
        'Negative caching, where misses are remembered too',
        'A cold start, as after a fresh deploy',
      ],
      answer: 1,
      explain: 'A cache stampede (thundering herd). Many simultaneous misses for the same key overwhelm the origin. Request coalescing, jittered TTLs and refresh-ahead are common fixes.',
    },
    {
      q: 'Why must Redis not be snip’s source of truth?',
      options: [
        'Redis cannot store URLs longer than a few hundred characters',
        'It’s memory-first: it may lose recent writes in a crash, and evicts keys when full',
        'Redis is too slow for writes, even though it is fast for reads',
        'Redis has no way to make a key last forever; every key must expire',
      ],
      answer: 1,
      explain: 'Redis is memory-first: depending on configuration it can lose recent writes on a crash and evicts keys when memory is full. Caches may forget things at any time — that is their contract. Durable, correct data lives in Postgres; Redis holds copies that can be rebuilt.',
    },
  ]}
/>

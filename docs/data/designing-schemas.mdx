  ...
  questions={[
    {
      ...
      options: [
        'Text primary keys are always best',
        'Slugs must be unique, never change, and every redirect looks up by slug — so the key gives the hottest query a perfect index',
        'Postgres does not support numeric keys',
        'To save disk space',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Copies the API key into each link',
        'Guarantees every owner_id exists, and deletes a key’s links when the key is deleted',
        'Encrypts the owner id',
        'Makes owner_id optional',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'It uses too little disk',
        'An update anomaly: changing the email means updating many rows, and missing one leaves conflicting data',
        'Emails cannot be stored in Postgres',
        'It makes JOINs impossible',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'A tags column holding a comma-separated list',
        'A join table (link_tags) with one row per link–tag pair and foreign keys to both',
        'A foreign key from tags to links',
        'Duplicate the link row once per tag',
      ],
      answer: 1,

  ...
  questions={[
    {
      ...
      answer: 1,
      explain: 'Slugs must be unique, never change, and every redirect looks up by slug — so the key gives the hottest query a perfect index. A natural key works well when it is truly unique, stable and the main lookup. If slugs could be renamed, a surrogate id plus a UNIQUE slug would be safer.',
    },
    {
      ...
      answer: 1,
      explain: 'Guarantees every owner_id exists, and deletes a key’s links when the key is deleted. The foreign key prevents references to missing rows; CASCADE says deleting the parent also deletes its children. RESTRICT or SET NULL are the alternatives.',
    },
    {
    ...
    },
    {
      ...
      answer: 1,
      explain: 'A join table (link_tags) with one row per link–tag pair and foreign keys to both. Many-to-many needs a junction table. Comma-separated lists cannot be indexed, validated or joined properly.',
    },
  ]}
/>

  ...
  questions={[
    {
      ...
      options: [
        'The query is as fast as it can be, since only 20 rows came back',
        'It read the whole table and threw nearly all of it away: add an index',
        'The table is corrupted, and Postgres is skipping the damaged rows',
        'The LIMIT is too small, so Postgres filtered extra rows to be safe',
      ],
      answer: 1,
      explain: 'Postgres read the whole table and discarded almost all of it — an index on the filtered column would likely help. A sequential scan that throws away nearly every row is the classic sign of a missing index for that WHERE clause.',
    },
    {
      q: 'snip has an index on (owner_id, created_at DESC). Which query can use it well?',
      options: [
        'WHERE created_at > ‘2026-09-01’ ORDER BY created_at DESC',
        'WHERE owner_id = 42 ORDER BY created_at DESC LIMIT 20',
        'WHERE url LIKE ‘%docs%’ ORDER BY created_at DESC',
        'WHERE clicks > 100 AND created_at > now() - interval ‘1 day’',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Postgres allows only one index per table, plus the primary key',
        'Each index slows writes and uses disk and memory; unused ones are pure cost',
        'Indexes make reads slower, because Postgres must pick one first',
        'Indexes on text columns lock the table on every read',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'A deadlock between the queries; take a table lock first',
        'The N+1 problem: fetch all owners at once, with a JOIN or ANY(...)',
        'Normal behaviour; 51 small queries are as fast as one big one',
        'A missing primary key, so each owner lookup scans the table',
      ],
      answer: 1,

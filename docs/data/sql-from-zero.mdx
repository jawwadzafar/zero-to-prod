  ...
  questions={[
    {
      ...
      options: [
        'The 3 oldest links that have more than 10 clicks',
        'Slugs of up to 3 links with over 10 clicks, most-clicked first',
        'Every link’s slug, sorted by clicks, stopping after 3 errors',
        'The total clicks of the 3 links with more than 10 each',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'NULL values cannot be stored, so the rows were never saved',
        'Comparing with NULL gives unknown, not true; use IS NULL',
        'The column holds empty strings, not NULL, so nothing matches',
        'Postgres skips NULL columns when it builds the result',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'A LIMIT; always add LIMIT 1 to updates',
        'A WHERE; write it as a SELECT first and check the rows',
        'An ORDER BY; updates without one change every row',
        'Nothing — UPDATE always changes every row in the table',
      ],
      answer: 1,
      explain: 'A WHERE clause; write it first as a SELECT and check the rows before updating. Without WHERE, UPDATE and DELETE affect the whole table. Selecting first shows exactly which rows will change; transactions let you undo before committing.',
    },
  ]}
/>

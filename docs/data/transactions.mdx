  ...
  questions={[
    {
      ...
      options: [
        'It stays — the INSERT already succeeded',
        'It is undone — the whole transaction is rolled back (atomicity)',
        'It is moved to another table',
        'Postgres asks the user',
      ],
      answer: 1,
      explain: 'Atomicity means all or nothing. Any failure before COMMIT undoes every change in the transaction.',
    },
    {
      ...
      options: [
        'Nothing — Read Committed prevents it',
        'A lost update: both read the same value and one increment disappears',
        'A dirty read',
        'The table is locked forever',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Checking is too slow',
        'Check-then-insert races; the PRIMARY KEY lets the database accept exactly one and reject the other atomically',
        'Postgres cannot check for existing rows',
        'Duplicate slugs are allowed',
      ],
      answer: 1,
      explain: 'Two requests could both see "free" and both insert. The unique constraint is the only race-free judge; snip maps the duplicate error to 409.',
    },
    {
      q: 'Postgres reports "deadlock detected". What should the application do?',
      options: [
        'Crash, since data is corrupted',
        'Retry the aborted transaction (with a limit), and prevent future deadlocks by locking rows in a consistent order',
        'Disable locking',
        'Switch to a different database',
      ],
      answer: 1,
      explain: 'Postgres aborts one transaction so the other can finish; nothing is corrupted. The victim should be retried, and consistent lock ordering plus short transactions make deadlocks rarer.',
    },
  ]}
/>

  ...
  questions={[
    {
      ...
      answer: 1,
      explain: 'Opening connections is slow and the database supports a limited number, so a few are opened once and shared. A pool reuses a small set of connections, avoiding handshake costs on every request and keeping the total within the database’s limit.',
    },
    {
    ...
    },
    {
      ...
      answer: 1,
      explain: 'Translates it to links.ErrSlugTaken, which becomes 409 Conflict (or triggers a retry for random slugs). The store translates database-specific errors into domain errors, keeping pgx details out of the business logic and the HTTP layer.',
    },
    {

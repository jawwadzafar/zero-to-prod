  ...
  questions={[
    {
      ...
      options: [
        'Postgres only allows one connection',
        'Opening connections is slow and the database supports a limited number, so a few are opened once and shared',
        'Pools encrypt the connection',
        'Go requires pools',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Go escapes quotes automatically in strings',
        'The query structure and the value are sent separately, so the value can only ever be data',
        'Postgres rejects strings containing quotes',
        'Slugs are validated by a regular expression',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Returns it to the client as a 500',
        'Translates it to links.ErrSlugTaken, which becomes 409 Conflict (or triggers a retry for random slugs)',
        'Ignores it',
        'Deletes the existing link',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        ...
        'It takes a Postgres advisory lock before migrating, and runs each migration in a transaction',
        'It waits a random amount of time',
        'Only the first copy connects to the database',
      ],
      answer: 1,

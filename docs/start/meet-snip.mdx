  ...
  questions={[
    {
      ...
      options: [
        'The web page the long link points to, fetched by snip and passed along',
        'A redirect: a response telling the browser to go to the long address',
        'A list of every link snip knows, so the browser can pick the right one',
        'A copy of the short code, which the browser then looks up on its own',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Because databases cannot add numbers safely while other requests are reading',
        'So the visitor is redirected at once and never waits for the counting',
        'Because a queue keeps the click data encrypted until the worker reads it',
        'Because the worker is written in a different language from the redirect code',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Copy each file into your project first, then read the chapter to understand it',
        'Ignore it completely; the chapters contain everything the reference contains',
        'Write your own version from the chapter, then compare it with the reference',
        'Only open it after finishing the whole handbook, as a final exam of sorts',
      ],
      answer: 2,

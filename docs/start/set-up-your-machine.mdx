...
The Go in `apt` is often out of date, so install the official release. Find the current version at [go.dev/dl](https://go.dev/dl) (for example `1.25.0`) and put it in the first line:

```bash
GO_VERSION=1.25.0                                                   # latest from go.dev/dl
...
<Quiz
  ...
  questions={[
    ...
    },
    {
      ...
      options: [
        'Write and compile Go code faster by caching every build between runs',
        'Run software like a database in a sealed box, without installing it for good',
        'Save snapshots of your code, so you can return to any earlier version',
        'Host your finished website on the internet for free, with no account needed',
      ],
      answer: 1,
    ...
    },
    {
      ...
      options: [
        'Windows has no terminal of its own, so a separate one has to be installed',
        'Servers almost always run Linux, so every command works as written',
        'WSL runs every program faster than Windows does, including the editor',
        'Go programs only compile and run on Linux, not on Windows itself',
      ],
      answer: 1,
      explain: 'Servers almost always run Linux, so learning on Linux means every command works as written. Windows has terminals, and Go runs on Windows. But the servers you will deploy to run Linux, and this handbook assumes Linux commands. WSL gives you real Linux alongside Windows.',
    },
    {
      q: 'A command fails with a long red error. What is the most effective first move?',
      options: [
        'Try the fixes you remember from last time until one of them works',
        'Read the error to the end, then search for the exact message in quotes',
        'Ignore it and run the next command; later steps often repair earlier ones',
        'Delete the project folder and start the chapter again from the top',
      ],
      answer: 1,
